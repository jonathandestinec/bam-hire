...
const AboutMenu = [{text:"Who We Are", link:"whoweare"}, {text:"Office Locations", link:"officelocations"}, {text:"Our Impact", link:"ourimpact"}]
...
function Nav() {
  ...
  return (
    <nav className='fixed inset-0 bg-[#00283c] text-white h-[100px]  flex items-center p-[15px] justify-between'>
        <a href="/"><h1 className='font-bold'>Logo</h1></a>
        <ul className='hidden nav-ul:flex items-center justify-between  w-[50%]'>
            ...
            {showJobSeekerMenu && <div className='absolute flex flex-col z-10 bg-[#00283c] top-[100%] left-[-20%] w-[200px] p-[20px] rounded-bl-[10px] rounded-br-[10px] '>
                {JobSeekersMenu.map((item, index)=>{
                    return <a href={item.link} className='mb-[15px] font-bold cursor-pointer' key={index}>{item.text}</a>
                })}
            </div>}
            </div>
            <div className='relative'>
        ...
        {!showMenu?<RiMenu3Line onClick={()=>{setShowMenu(!showMenu)}} className='block nav-ul:hidden cursor-pointer text-xl'  />:<MdClose onClick={()=>{setShowMenu(!showMenu)}} className='block nav-ul:hidden cursor-pointer text-xl' />}
        {
            showMenu && <div className='absolute top-[100%] left-0 right-0 flex nav-ul:hidden bg-[#00283c] z-10 p-[15px]'>
            <ul className='w-full'>
                <a href="/"><li className='flex items-center w-full justify-between mb-[20px] font-bold'>Home</li></a>
                <div>
                ...
                {showJobSeekerMenu && <div className='flex flex-col'>
                    {JobSeekersMenu.map((item, index)=>{
                        return <a href={item.link} className='mb-[15px]' key={index}>{item.text}</a>
                    })}
                </div>}
                </div>
                <div>
                ...
                <a href="contact"><li className='flex items-center w-full justify-between mb-[20px] font-bold'>Contact</li></a>
            </ul>
        </div>
        }
    </nav>
